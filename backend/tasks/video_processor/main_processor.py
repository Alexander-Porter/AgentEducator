...
def process_video_task(video_id, stop_flag=None, processing_steps=None, preview_mode=False):
    ...
    try:
        ...
        # 检查当前处理状态
        current_status = check_video_processing_steps_status(video_id)
        add_task_log(task_id, video_id, 'info', f"当前处理状态: {current_status}")
        ...
        if "keyframes" in processing_steps:
            ...
            # 检查是否需要重新提取关键帧
            should_extract = not current_status["keyframes"]
            if should_extract:
                ...
                add_task_log(task_id, video_id, 'info', "关键帧已存在，跳过提取步骤")
                # 从数据库加载已有的关键帧数据
        ...
        if "ocr" in processing_steps:
            ...
            should_process_ocr = not current_status["ocr"]
            if should_process_ocr:
                ...
                add_task_log(task_id, video_id, 'info', "OCR已完成，跳过OCR处理步骤")
                # 确保加载了OCR数据
                if not any("ocr_result" in kf for kf in keyframes_data):
                    keyframes_data = load_keyframes_from_db(video_id, include_ocr=True, include_asr=False)
        ...
        if "asr" in processing_steps:
            ...
            should_process_asr = not current_status["asr"]
            if should_process_asr:
                ...
                add_task_log(task_id, video_id, 'info', "ASR已完成，跳过ASR处理步骤")
                # 确保加载了ASR数据
                if not any("asr_texts" in kf and kf["asr_texts"] for kf in keyframes_data):
                    keyframes_data = load_keyframes_from_db(video_id, include_ocr=True, include_asr=True)
        ...
        if "vector" in processing_steps:
            ...
            vector_exists = current_status["vector"]
            if not vector_exists:
                ...
                add_task_log(task_id, video_id, 'info', "向量索引已存在，跳过构建步骤")
        ...
        if "summary" in processing_steps:
            ...
            summary_exists = current_status["summary"]
            if not summary_exists:
                ...
                add_task_log(task_id, video_id, 'info', "视频摘要已存在，跳过生成步骤")
